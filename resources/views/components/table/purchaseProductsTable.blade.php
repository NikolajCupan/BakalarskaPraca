...
<table class="purchaseProductsTable mt-4 table table-striped display nowrap" style="width: 100%">
    ...
    @foreach ($basketProducts as $basketProduct)
        @php ($price = $basketProduct->getPriceOfDate($purchaseDate)->price)
        <tr>
            <td>
            ...
            @if ($withReclaimButton && Auth::user()->hasRole(['purchaseManager']) && $purchaseStatus != "cancelled")
                <td>
                    <a class="productReclaimModalOpen" href="" data-bs-toggle="modal" data-bs-target="#productReclaimModal"
                       ...
                       data-product-quantity="{{$basketProduct->quantity}}"
                       data-product-id="{{$basketProduct->getProduct()->id_product}}">
                        <svg class="ms-1 bi bi-arrow-repeat" xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" viewBox="0 0 16 16">
            ...
            @endif
            <td>
                <a href="{{$path . $basketProduct->getProduct()->id_product}}">
